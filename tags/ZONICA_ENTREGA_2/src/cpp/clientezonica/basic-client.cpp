...
int ReadReply(SOCKET sd, char* data, int dataLen);
...
int DoWinsock(const char* pcHost, int nPort, char *data, int dataLen)
{
    ...
    cout << "Sending packet (" << strlen(data) << " bytes)..." << flush;
    int nBytes;
	// Send the string to the server
    if (send(sd, data, dataLen, 0) != SOCKET_ERROR) 
	{
		
		cout << endl;
		if ((nBytes = ReadReply(sd, data, dataLen)) > 0) 
		{
		...
		{
			cerr << endl << "Server unexpectedly closed the connection" << endl;
		}
		else 
		{
			cerr << endl << WSAGetLastErrorMessage("read reply") <<	endl;
			return 3;
		}
	}
	else 
...
int ReadReply(SOCKET sd, char *data, int datalen)
{
    ...
    if (strncmp(acReadBuffer, data, nTotalBytes) == 0) 
	{
