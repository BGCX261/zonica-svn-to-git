...
namespace Zonica.DAO {
	...
	public class EscuchaDao : Dao, IEscuchaDao {
		...
		/// <see cref="IEscuchaDao.RetrieveAmigos(int)" />
		public IList RetrieveAmigos(int idUsuario) {
			return this.HibernateTemplate.FindByNamedQueryAndNamedParam("amigosMusicalesDelUsuario", "idUsuario", idUsuario);
		}
		...
		/// <see cref="IEscuchaDao.RetrieveUltimasEscuchas(int, int)" />
		public IList RetrieveUltimasEscuchas(int idUsuario, int cantidad) {
			IQuery query = this.HibernateTemplate.Session.GetNamedQuery("ultimasEscuchasPorUsuario");
