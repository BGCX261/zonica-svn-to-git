...
namespace Zonica.Importer {
	...
	public class MusicaImporter {
		...
		private static void Main(string[] args) {
			...
			try {
				using (IApplicationContext ctx = ContextRegistry.GetContext()) {
					...
					log.InfoFormat("Importando {0} archivos de {1}.", fileInfos.Length, fd.FullName);
					foreach (FileInfo fileInfo in fileInfos) {
						...
						try {
							...
							try {
								escuchaService.RegistrarEscucha(
									datosDeEscucha[2],
									datosDeEscucha[3],
									StringUtils.ToCamelCasePhrase(datosDeEscucha[4]),
									StringUtils.ToCamelCasePhrase(datosDeEscucha[6]),
									StringUtils.ToCamelCasePhrase(datosDeEscucha[5]),
									momentoInicio, 1);
							} catch (ZonicaBaseException e) {
								log.WarnFormat("Error de procesamiento del archivo {0}: {1}", fileInfo.Name, e.Message);
							} catch (ArgumentException e) {
								log.WarnFormat("Error de datos al procesar el archivo {0}: {1}", fileInfo.Name, e.Message);
							}
						} catch (Exception e) {
							log.ErrorFormat("Error procesando el archivo {0}: {1}", fileInfo.Name, e.Message);
						} finally {
		...
		private static IDictionary GetParameters(string[] args) {
			...
			if (args.Length < 2) {
				ImprimirAyuda();
				log.WarnFormat("Importador iniciado con parámetros incorrectos.");
				return null;
			}
